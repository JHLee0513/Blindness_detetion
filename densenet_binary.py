...
for cv_index in range(1,6):
    fold = cv_index
    log_fold = cv_index
    qwk_ckpt_name = '/nas-homes/joonl4/blind_weights/raw_densenet_pretrained_binary_smoothen_fold'+str(fold)+'.h5'
...
#model = Xception(include_top = False, weights = 'imagenet', input_shape = (img_target,img_target,3), pooling = 'max')
    model = DenseNet121(input_shape = (img_target, img_target, 3), weights = 'imagenet', include_top = False, pooling = 'avg')
    ...
    save_model_name = '/nas-homes/joonl4/blind_weights/raw_densenet_pretrained_binary_smoothen_fold'+str(fold)+'.hdf5'
    model_checkpoint = ModelCheckpoint(save_model_name,monitor= 'val_loss',
                                    mode = 'min', save_best_only=True, verbose=1,save_weights_only = True)
    #csv = CSVLogger('./raw_effnet_pretrained_binary_fold'+str(fold)+'.csv', separator=',', append=False)
    cycle = 2560/batch * 5
    cyclic = CyclicLR(mode='exp_range', base_lr = 0.0001, max_lr = 0.002, step_size = cycle)  
